// ============================================================================
//  PRONTUÁRIO RÁPIDO - SERVIDOR (BACKEND) - v7.8
//
...
//  'pensarMais' (v7.9, toggle da tela): DESLIGADO (padrão) = sem a etapa extra
//  de pensamento do Opus 5, mais parecido com o 4-8 de antes, teto menor.
//  LIGADO = pensamento em esforço médio, para os casos que você julgar mais
//  complexos — teto maior, mas só custa mais se o modelo REALMENTE pensar
//  mais (o teto em si é de graça). Decisão sua, caso a caso, na tela.
async function chamarClaude(promptFinal, imagens, sinal, pensarMais) {
    if (!process.env.ANTHROPIC_API_KEY) {
    ...
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
            model: MODELO_CLAUDE,
            // v7.9: o Opus 5 pensa por padrão e o pensamento conta DENTRO deste
            // teto (pensamento + JSON). Por isso o teto sobe nos dois casos —
            // subir o teto não custa nada a mais sozinho, só dá fôlego.
            max_tokens: pensarMais ? 24000 : 12000,
            // Sem pensarMais: desliga a etapa de pensamento (thinking), ficando
            // mais parecido com o comportamento de antes (4-8, sem pensar).
            // Com pensarMais: deixa pensar, em esforço médio (nem o mínimo,
            // nem o máximo) — ainda não medido no seu tipo de caso; é o que o
            // log de custo vai mostrar na prática.
            ...(pensarMais
...
//   - pensarMais (v7.9): repassado ao Claude quando é ele quem responde
//     (direto ou por fallback). Não se aplica ao Gemini.
async function gerarComFallback(promptFinal, modeloId, usarBusca, imagens, sinal, pensarMais) {
    if (modeloId === 'claude') {
        const r = await chamarClaude(promptFinal, imagens, sinal, pensarMais);
        return {
...
app.post('/api/atendimento', limitarAbuso, async (req, res) => {
    ...
    try {
        ...
        // v7.9 — toggle da tela: pensamento extra do Claude (esforço médio),
        // caso a caso. Desligado por padrão. Só tem efeito quando quem
        // responde é o Claude (direto ou por fallback do Gemini).
        const pensarMais = op.pensarMais === true;
        ...
        if (compararIAs) {
            ...
            try {
                ...
                [rGemini, rClaude] = await Promise.all([
                    ...
                    chamarClaude(promptFinal, fotos, abortador.signal, pensarMais)
                ]);
            } catch (e) {
...
${JSON.stringify(dadosClaude)}`;
        ...
        try {
            saida = await gerarComFallback(promptFinal, modeloId, usarBusca, fotos, abortador.signal, pensarMais);
...
const VERSAO_SERVIDOR = 'v7.9';
app.get('/', (req, res) => res.send(
    'Servidor do Prontuario Rapido ' + VERSAO_SERVIDOR + ' no ar. '
    + 'Linhas esperadas no server.js: 2782.'
));
