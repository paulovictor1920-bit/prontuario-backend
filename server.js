// ============================================================================
//  PRONTUÁRIO RÁPIDO - SERVIDOR (BACKEND) - v8.1
//
...
function preferirPrimeiraPessoaConduta(txt) {
    ...
    const VERBOS_DE_FECHO = /^\s*(?:[-•*]\s*|\d+[.)]\s*)?(reavaliar|avaliar|acompanhar|observar|conferir|vigiar|aguardar|manter)\b/i;
...
async function gerarComFallback(promptFinal, modeloId, usarBusca, imagens, sinal) {
    ...
    try {
        const r = await chamarGemini(promptFinal, modeloId, usarBusca, imagens, sinal);
        return {
...
Neurológico: ECG 15, pupilas isocóricas e fotorreagentes, pares cranianos preservados, força e sensibilidade preservadas e simétricas nos 4 membros, sem sinais de irritação meníngea"
   ...
   - "conduta": condutas e orientações. SEJA CONCISO — nada de "encher
     ...
     ÚLTIMO ITEM NO INFINITIVO: o último item da conduta é sempre o de
     reavaliação/acompanhamento, e SÓ ele vai no INFINITIVO — "Reavaliar após
     as medicações e os resultados dos exames.", "Acompanhar...", "Observar...",
     "Conferir...". Os demais itens ficam na primeira pessoa.
     PROIBIDO CONDICIONAL DE DESFECHO: a conduta registra o que está sendo
     feito AGORA e a reavaliação. É PROIBIDO escrever critério de alta, de
     liberação ou de desfecho antecipado — nada de "se melhorar, alta", "alta
     se estabilizar", "entrego a receita se houver liberação", "libero após
     reavaliação". O desfecho se decide depois e vira uma NOVA evolução. Isto
     NÃO proíbe sinais de alarme nem orientação de retorno na receita
     domiciliar, que continuam normais, no lugar deles.
...
const VERSAO_SERVIDOR = 'v8.1.2';
app.get('/', (req, res) => res.send(
    'Servidor do Prontuario Rapido ' + VERSAO_SERVIDOR + ' no ar. '
    + 'Linhas esperadas no server.js: 3253.'
));
