// ============================================================================
//  PRONTUÁRIO RÁPIDO - SERVIDOR (BACKEND) - v8.0.2
//
...
1. "discussao": este é o campo de RACIOCÍNIO CLÍNICO — o mais importante para o
   ...
   ESTRUTURA EM DOIS NÍVEIS (v8.0, regra central de formato):

   NÍVEL 1 — abra com a linha "O QUE MUDA A CONDUTA:" (este rótulo exato) e
   escreva de 4 a 8 linhas (até 10 SÓ se o caso for realmente complexo). Este
   bloco existe SEMPRE, em todo caso. É o que o médico lê no meio do plantão.
   O TETO É PARA VALER: se você escreveu 12 itens, corte até caber em 8 — o que
   sobrar é o que decide conduta, o resto é enchimento. Cada linha tem de
   DECIDIR alguma coisa; se a linha só descreve o quadro ou repete o
   prontuário, ela não entra. Entram aqui, conforme o caso:
     • ajuste, suspensão ou contraindicação de medicação diante do quadro atual
       (função renal, lactato, interação, alergia, idade);
     • alvo terapêutico particular DESTE paciente (ex.: alvo de saturação
       92-96% por suspeita de DPOC não diagnosticada);
     • achado do histórico ou do anexo que muda a decisão (ex.: passagem prévia
       pela unidade, exame anterior, receita trazida);
     • o que MUDARIA A CONDUTA se um exame vier alterado (ex.: "se o ECG
       mostrar supra, isto deixa de ser área verde");
     • dose que precisa de conferência e risco que exige vigilância específica;
     • a justificativa da escolha terapêutica, quando não for óbvia.

   NÍVEL 2 — bloco OPCIONAL, encabeçado pela linha "DIFERENCIAIS:" (este rótulo
   ...
   quadro que pode ser duas coisas diferentes. ATENÇÃO: gravidade NÃO é
   ambiguidade. Um paciente hipoxêmico com pneumonia evidente é GRAVE, mas o
   diagnóstico não está em aberto — nesse caso o bloco DIFERENCIAIS não existe,
   e a gravidade é tratada no nível 1 e, se couber, na caixa vermelha. Quando existir, para CADA diferencial diga o que no caso o
   FAVORECE ou AFASTA — raciocine, não liste — e inclua os "CAN'T MISS" daquele
   quadro e como descartá-los. Em quadro clássico e coerente (pneumonia típica,
   IVAS viral, lombalgia mecânica sem red flag), NÃO escreva este bloco:
   descartar TEP, síndrome coronariana e insuficiência cardíaca num caso que
   não os sugere gasta espaço e não ajuda ninguém.
...
const VERSAO_SERVIDOR = 'v8.0.2';
app.get('/', (req, res) => res.send(
    'Servidor do Prontuario Rapido ' + VERSAO_SERVIDOR + ' no ar. '
    + 'Linhas esperadas no server.js: 3039.'
));
