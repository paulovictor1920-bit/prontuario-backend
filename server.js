// ============================================================================
//  PRONTUÁRIO RÁPIDO - SERVIDOR (BACKEND) - v8.2
//
//  v8.2 (11/set/2026) — TRÊS CORREÇÕES, todas nascidas do log de plantão.
//
//    1) FLASH "PAROU DE FUNCIONAR" — NÃO FOI MUDANÇA NOSSA. O log do Render
//       mostra, sempre, a MESMA resposta do servidor do Google:
//       "[503] This model is currently experiencing high demand. Spikes in
//       demand are usually temporary." Isso é capacidade do lado do Google, não
//       erro de pedido — e no dia 11/set o MESMO 503 pegou também o
//       gemini-2.5-pro (botão Pro), que não foi tocado em nenhuma versão
//       recente. Erro de código nosso não escolhe modelo assim, e o Flash
//       respondeu normalmente em várias gerações do mesmo dia (29,6 s às
//       14h20). O que ERA nosso: ao tomar um 503, o servidor desistia do Gemini
//       NA HORA e ia direto para o Claude — trocando uma geração de R$ 0,10 por
//       uma de R$ 0,90, quando o próprio Google diz que o pico é temporário.
//       AGORA EXISTE UMA ESCADA, antes do Claude:
//         (a) NOVA TENTATIVA no mesmo modelo, depois de uma pausa curta
//             (1,2 s e depois 3 s). 503 costuma passar em segundos.
//         (b) Se o motor era o Flash e as duas tentativas falharam, tenta o
//             FLASH DE RESERVA (gemini-3.7-flash), que o Google mantém em
//             suporte pleno e que custa EXATAMENTE O MESMO (US$ 0,75 / 3,75 por
//             milhão até 31/dez/2026). Custo por atendimento: idêntico.
//         (c) Só então cai para o Claude, como antes.
//       Tentativa que FALHA não é cobrada, então a escada não gasta nada; ela
//       ECONOMIZA, por evitar o Claude. Tudo respeita o cancelamento e o teto de
//       2 minutos: as tentativas extras só acontecem se ainda houver tempo
//       (corte em 60 s de relógio) e a pausa é interrompida pelo botão cancelar.
//
//    2) "REAVALIAR" NO ÚLTIMO ITEM MESMO COM ALTA JÁ DECIDIDA PELO MÉDICO.
//       A culpa era do prompt, e era literal: estava escrito "o último item da
//       conduta é SEMPRE o de reavaliação/acompanhamento". Junto com o bloco
//       "PROIBIDO CONDICIONAL DE DESFECHO" (que existe por bom motivo: proíbe
//       "se melhorar, alta"), a IA aprendia a NUNCA registrar alta e a SEMPRE
//       fechar com "Reavaliar...". Mas alta que o médico JÁ DECIDIU não é
//       condicional — é fato do atendimento, e tem que constar.
//       Agora o prompt separa os dois casos e o servidor DETECTA no relato, de
//       forma determinística, quando o médico declarou a alta (função
//       'medicoDeclarouAlta'), injetando uma instrução curta e específica.
//       ATENÇÃO — decisão que eu tomei e que precisa do seu aval: o servidor
//       NÃO reescreve nem apaga o último item da conduta. Apagar destruiria
//       orientação legítima ("Reavaliar em 48 h na UBS" é correto em alta), e
//       reescrever seria inventar texto clínico. A trava é de prompt reforçado.
//
//    3) "JSON INVÁLIDO" DO CLAUDE — segunda causa encontrada. O reparo da
//       v8.1.1 só conserta QUEBRA DE LINHA crua. O log de 11/set mostra duas
//       respostas que começam e terminam certo ({"discussao": ... "relatorio":
//       ""}) e mesmo assim não abrem: sobra a ASPA SOLTA dentro do texto (a IA
//       escreve termo entre aspas na discussão e esquece a barra invertida).
//       Nova função 'repararAspasSoltas', usada só DEPOIS de todas as leituras
//       normais falharem: ela escapa a aspa que não fecha nada, sem apagar um
//       caractere sequer. E o log de falha passou a dizer a MENSAGEM do erro e
//       a mostrar o trecho exato onde a leitura travou.
//
//  v8.1.2 (09/set/2026) — CORREÇÃO URGENTE: o Claude parou de funcionar por
//    inteiro. Toda chamada voltava em 0,3 s com "Claude HTTP 400 ... This model
//    does not support assistant message prefill. The conversation must end with
//    a user message." A causa é NOSSA, não da Anthropic: o PREFILL "{" que a
//    v8.1.1 acrescentou (uma mensagem de role 'assistant' no fim da conversa)
//    NÃO é aceito pelo claude-opus-5. O modelo recusa o pedido antes de gerar
//    qualquer coisa — por isso falhava sempre, e não de vez em quando.
//    O prefill FOI REMOVIDO (e o "{" que era recolocado na leitura, junto).
//    As outras DUAS medidas da v8.1.1 contra o "JSON inválido" CONTINUAM: a
//    regra explícita no system sobre escapar a quebra de linha e o reparo
//    determinístico no extrator. O preâmbulo e a cerca de código, que o prefill
//    prevenia, já eram tratados pelo extrairJson desde antes (ele limpa ```json
//    e recorta do primeiro "{" ao último "}").
//
//  v8.1.1 (08/set/2026) — duas correções pedidas depois do teste de campo:
//    1) "JSON inválido" INTERMITENTE, só no Claude (falhava na 1ª tentativa e
//       passava na 2ª, mesmo relato). Causa provável: o Gemini roda em modo JSON
//       nativo (responseMimeType) e não erra nisso; o Claude escreve o JSON à
//       mão, e o NOSSO prompt manda usar "quebra de linha real" no exame físico
//       e na conduta — o que, dentro de uma string, invalida o arquivo inteiro.
//       Três medidas: (a) PREFILL "{" na resposta do Claude, que elimina
//       preâmbulo e cerca de código; (b) regra explícita no system sobre escapar
//       a quebra de linha; (c) REPARO determinístico no extrator, que escapa a
//       quebra crua sem apagar conteúdo nenhum. Mais o log de diagnóstico, que
//       agora diz motor, motivo da parada, tamanho e as pontas do texto.
//    2) DISCUSSÃO REESCRITA. O formato de dois níveis da v8.0 ficou travado e
//       quase não analisava a conduta do médico. Agora o bloco principal é
//       "SOBRE A SUA CONDUTA:" — concorda? acrescentaria? retiraria? — e os
//       blocos "O QUE MAIS MUDA A CONDUTA:" e "DIFERENCIAIS:" ficaram opcionais.
//       O aviso de medicação fora da padronização saiu do foco: sumiu da receita
//       domiciliar e virou uma linha curta no fim para o que é executado dentro
//       da unidade (isto MODIFICA a regra 6, a pedido do médico).
//
//  v8.1 (08/set/2026) — FATIA Q: A INTERNAÇÃO VIRA UM TIPO DE DOCUMENTO.
//    - Antes: toggle 🏥 dentro do Prontuário/Evolução, que empilhava os cinco
//      documentos de internação NA MESMA geração. Saída de ~10.200 tokens,
//      atenção dividida e campos 20/21/22 inchados. O toggle SAIU.
//    - Agora: tipoDocumento === 'internacao' é uma ABA própria, ao lado de
//      Prontuário e Evolução. Gera o PRONTUÁRIO ÚNICO DE INTERNAÇÃO — os mesmos
//      campos do SIGRAH, consolidando o caso inteiro — mais os campos 20/21/22
//      da AIH e a prescrição de internação. Duas gerações menores, cada uma com
//      o modelo inteiro prestando atenção numa coisa só.
//    - "relatorio_internacao" DEIXOU DE EXISTIR: os campos do prontuário SÃO o
//      relatório. A chave continua sendo zerada à força, para o caso de uma tela
//      antiga ainda esperar por ela.
//    - A aba NÃO exige relato novo. Fontes, nesta ordem: histórico do BE (72 h),
//      anexos (fotos/PDFs) e o campo de instrução curto. Se as três estiverem
//      vazias, o servidor RECUSA com mensagem clara (regra 15).
//    - Campos 20, 21 e 22 ganharam TETO explícito de linhas e palavras — a
//      queixa de que o Claude escreve demais, tratada como no relatório APS.
//    - Na aba, "evolucao" é zerada à força: o documento é de prontuário.
//
//  v8.0.2 (07/set/2026) — o Flash devolveu "JSON inválido" DUAS vezes no teste
//  com o modo internação ligado (26,2 s e 23,6 s). NÃO era falta de capacidade
//  do modelo: era o TETO DE SAÍDA de 8192 do nosso próprio código. A mesma
//  geração, feita pelo Claude, consumiu ~10.200 tokens de saída (calculado a
//  partir do custo de R$ 1,558) — ou seja, o documento com internação NÃO CABE
//  em 8192 e o Flash era cortado no meio do JSON.
//    - Gemini: maxOutputTokens 8192 -> 32768 (o 3.8-flash e o 2.5-pro aceitam
//      65.536). Nos modelos Gemini 3 esse teto é orçamento COMBINADO de
//      pensamento + resposta, o que apertava ainda mais.
//    - Claude: max_tokens 12000 -> 24000 (usou 10.200, 85% do teto anterior).
//    - Subir teto não custa nada sozinho: cobra-se o que o modelo usar.
//    - RELATÓRIO APS ENXUTO: no máximo 8 linhas (~130 palavras), sem repetir o
//      exame físico, a lista de sinais vitais nem os exames pedidos. O que
//      interessa ao colega da APS é o que ele precisa FAZER daqui em diante.
//
//  v8.0.1 (07/set/2026) — três correções vindas do PRIMEIRO teste de campo da
//  v8.0, feito nos três motores (Claude, Flash e Pro) na UPA Acrízio:
//    1) O ÚLTIMO ITEM DA CONDUTA SAÍA "Reavalio" NOS TRÊS MOTORES. A culpa NÃO
//       era da IA: a função preferirPrimeiraPessoaConduta (do próprio servidor,
//       da v6) convertia "Reavaliar" -> "Reavalio" à força, desfazendo a regra
//       nova. Agora o verbo de fecho do ÚLTIMO item é preservado. Lição: regra
//       nova de prompt pode brigar com função determinística antiga — conferir
//       o pós-processamento sempre que uma regra de formato mudar.
//    2) A MESMA função deixou de "consertar" os verbos que a v8.0 proibiu
//       (administrar, fazer, aplicar, puncionar, monitorizar, coletar,
//       transferir): ela transformava "Administrar dipirona" em "Administro
//       dipirona", ou seja, produzia exatamente o verbo de enfermagem proibido.
//    3) Reforço de duas regras que escaparam no Gemini Pro: a conduta não
//       detalha exames nem entre parênteses, um item da conduta não empilha
//       quatro ações, o nível 1 da discussão tem teto real de 8 linhas, e
//       GRAVIDADE não é AMBIGUIDADE (pneumonia hipoxêmica evidente não abre
//       bloco de DIFERENCIAIS).
//
//  Novidades da v8.0 (07/set/2026) — fatias I, J, L, M, N, O e P do roteiro.
//  Sessão de código escrita a partir dos oito testes de campo de 07/set.
//
//    I) CONDUTA — VERBOS E FORMATO (prompt):
//       - verbos do MÉDICO, não da enfermagem: "prescrevo/solicito/oriento/
//         encaminho/mantenho/indico/reavalio". PROIBIDO "instalo",
//         "administro", "punciono", "ofereço", "coleto", "infundo" e "faço"
//         (no sentido de aplicar). Quem instala cateter é a enfermagem.
//       - INTERNAÇÃO: o médico da UPA SOLICITA a internação e PREENCHE a
//         documentação. A vaga depende da coordenação de vagas e da conferência
//         dos documentos — não está sob o controle de quem escreve. Proibido
//         "interno o paciente", "aciono vaga", "transfiro para o hospital".
//       - último item da conduta no INFINITIVO ("Reavaliar após...").
//       - PROIBIDO condicional de desfecho ("se melhorar, alta"): risco
//         médico-legal de predisposição documentada antes do exame.
//       - conduta não repete a lista de exames (cita por categoria).
//       - o MODELO de conduta foi trocado: o anterior ensinava justamente o
//         que agora é proibido ("Faço na UPA", "se melhora, alta").
//    J) PEDIDO DE EXAME COM TRAÇOS: regra no prompt + função determinística
//       tracosNoPedidoExame, aplicada SÓ a exame_unidade/exame_externo. No
//       teste de campo o Flash numerava e os outros dois não.
//    L) A IA NÃO ESCREVE DILUIÇÃO: proibição explícita na prescrição interna E
//       na de internação. O servidor não escreve por cima de diluição já
//       escrita — então uma diluição da IA APAGAVA a reconstituição da tabela e
//       o preparo saía pela metade (aconteceu com a ceftriaxona no Pro).
//    M) RECEITA DOMICILIAR:
//       - USO CONTÍNUO NUNCA entra na receita, salvo pedido expresso do médico.
//         A lista informada de boca pelo paciente é pouco confiável e a
//         renovação é competência de quem o acompanha na atenção primária.
//       - com internação indicada, nada de tratamento de casa para a doença
//         aguda; a receita pode ficar só com o bloco ORIENTAÇÕES.
//    N) PENSAMENTO EXTRA DO CLAUDE REMOVIDO (servidor e tela). Ligado, produziu
//       receita incoerente em 2 de 2 casos e uma falha de JSON. Custo nunca foi
//       o argumento. max_tokens fixo em 12000, thinking sempre 'disabled'.
//    O) INJEÇÃO CONDICIONAL E TRAVAS DE BUSCA:
//       - chaves "exame_unidade"/"exame_externo" e os itens 7/8 do dicionário
//         só entram no prompt com o toggle ligado (mesmo padrão da internação);
//         some também a instrução negativa "NÃO gere pedido de exame".
//       - chave "fontes" só existe quando a busca está ligada, e o bloco
//         "FONTES CONSULTADAS" só é impresso nesse caso. Antes, o campo existia
//         sempre e a IA o preenchia de memória: atribuição FABRICADA.
//       - busca + Claude = busca IGNORADA e AVISO na tela (o Claude não tem
//         ferramenta de busca; o bloco pedia link a quem não pode procurar).
//    P) DISCUSSÃO EM DOIS NÍVEIS: "O QUE MUDA A CONDUTA:" (4 a 8 linhas, até 10
//       em caso complexo, sempre) e "DIFERENCIAIS:" (só em caso ambíguo).
//
//  A fatia K (faixa do motor estagnada ao reabrir um caso) foi corrigida no
//  index.html: mostrarAvisoMotor passou a ser chamada dentro de exibirResposta.
//
//  Novidades da v7.8 (07/set/2026) — MODELOS DE IA (fatia E do roteiro):
//    - FLASH: gemini-3.6-flash -> gemini-3.8-flash. Os três (3.6, 3.7 e 3.8)
//      custam o mesmo (US$ 0,75 / US$ 3,75 por milhão de tokens de entrada /
//      saída até 31/dez/2026), então a troca não muda o custo por atendimento.
//    - TEMPERATURE REMOVIDO das chamadas ao Gemini (era 0.4, valia para o Flash
//      E para o Pro). O Google recomenda manter o padrão de 1.0 em todos os
//      modelos Gemini 3: valores abaixo de 1.0 podem causar looping ou piora de
//      desempenho. A remoção foi feita para os DOIS botões por decisão do
//      médico (07/set), ciente de que o aviso do Google não alcança o
//      gemini-2.5-pro e de que o texto do Pro passa a variar um pouco mais
//      entre gerações do mesmo relato.
//    - PRO: continua em gemini-2.5-pro. Nada mudou ali além do temperature.
//    - Nada mais foi tocado: prompts, pós-processamento, diluições, internação,
//      histórico e rotas seguem idênticos à v7.7.
//
...
// ----------------------------------------------------------------------------
//  Modelos do Gemini.
//  FLASH atualizado (07/set/2026): gemini-3.8-flash (lançado 02/set/2026),
//  listado pelo Google como estável. Substitui o gemini-3.6-flash, que continua
//  estável e NÃO foi descontinuado — a troca é melhoria, não conserto. Preço
//  idêntico entre 3.6, 3.7 e 3.8, então o custo por atendimento não muda.
//  ATENÇÃO: 3.7 e 3.8 NÃO aceitam o nível de pensamento "minimal" (devolvem
//  erro); o 3.6 aceitava. Hoje o código não configura nível nenhum e o padrão
//  dos três é "médio", então não há nada a mudar — mas se um dia quisermos um
//  modo mais rápido e barato, o "minimal" não está disponível aqui.
...
const MODELO_RAPIDO = 'gemini-3.8-flash';
const MODELO_PROFUNDO = 'gemini-2.5-pro';

//  v8.2 — FLASH DE RESERVA. Só entra em cena quando o gemini-3.8-flash devolve
//  503 ("high demand") DUAS vezes seguidas. O 3.8 é modelo novo (02/set/2026) e
//  modelo novo é justamente onde a capacidade do Google aperta; o 3.7 continua
//  em suporte pleno e com o MESMO preço (US$ 0,75 / US$ 3,75 por milhão até
//  31/dez/2026), então usar a reserva NÃO muda o custo do atendimento.
//  SEGURANÇA DE NOME: se algum dia este nome sair do ar, a reserva simplesmente
//  falha e o fluxo segue para o Claude, exatamente como fazia antes da v8.2 —
//  um nome errado aqui não quebra nada, só gasta ~0,3 s. Para desligar a
//  reserva, basta deixar esta linha como string vazia ('').
const MODELO_RAPIDO_RESERVA = 'gemini-3.7-flash';
...
//  v7.9 (07/set/2026): opus-4-8 -> opus-5. Preço IGUAL ($5/$25 por milhão).
//  Diferença real: o Opus 5 pensa por padrão (ver PENSAR_MAIS mais abaixo e a
//  função chamarClaude). O 4-8 continua ativo — sem pressa se algum dia
//  precisar voltar, é só trocar esta linha de volta.
const MODELO_CLAUDE = 'claude-opus-5';

// ----------------------------------------------------------------------------
//  v7.9 — CUSTO ESTIMADO POR ATENDIMENTO (07/set/2026)
// ----------------------------------------------------------------------------
//  Mostra na tela, em REAIS, quanto custou cada geração. Preços conferidos na
//  documentação oficial nesta data — Claude e Gemini cobram "pensamento" como
//  token de SAÍDA, mesmo quando você nunca vê o texto do raciocínio.
//
//  TAXA DE DÓLAR: FIXA de propósito (decisão do médico) — o servidor NUNCA
//  consulta a cotação do dia. Atualize esta linha de vez em quando; não
//  precisa ser exata, é só uma referência de custo, não uma fatura.
const COTACAO_DOLAR_BRL = 5.15; // ajustada em 07/set/2026 (dólar rondando R$5,08-5,22)

const PRECOS_USD_POR_MILHAO_TOKENS = {
    claude:       { entrada: 5.00,  saida: 25.00 },  // claude-opus-5
    gemini_flash: { entrada: 0.75,  saida: 3.75 },   // gemini-3.8-flash, até 31/dez/2026
    gemini_pro:   { entrada: 1.25,  saida: 10.00 }   // gemini-2.5-pro, prompts < 200k tokens
};

//  Calcula o custo de UMA chamada. 'motor' = 'claude' | 'gemini_flash' | 'gemini_pro'.
//  Devolve null se não tiver como calcular (evita mostrar "R$0,00" enganoso).
function calcularCustoBRL(motor, tokensEntrada, tokensSaida) {
    const p = PRECOS_USD_POR_MILHAO_TOKENS[motor];
    if (!p || !Number.isFinite(tokensEntrada) || !Number.isFinite(tokensSaida)) return null;
    const usd = (tokensEntrada * p.entrada + tokensSaida * p.saida) / 1000000;
    return { usd: Number(usd.toFixed(4)), brl: Number((usd * COTACAO_DOLAR_BRL).toFixed(3)) };
}

//  Soma dois custos (usado quando mais de uma chamada ao Claude compõe a
//  mesma geração — ex: a chamada principal + o resumo da comparação).
function somarCustoBRL(a, b) {
    if (!a) return b || null;
    if (!b) return a;
    return { usd: Number((a.usd + b.usd).toFixed(4)), brl: Number((a.brl + b.brl).toFixed(3)) };
}
...
// v8.2 — O MÉDICO DECLAROU ALTA NESTE RELATO?
// Serve para um fim só: reforçar a instrução do prompt quando o desfecho JÁ foi
// decidido. Sem isso, a IA fechava a conduta com "Reavaliar após as
// medicações..." mesmo com o médico tendo escrito que ia dar alta — porque o
// prompt mandava, ao pé da letra, que o último item fosse SEMPRE a reavaliação.
//
// O QUE ESTA FUNÇÃO NÃO FAZ (de propósito): ela NÃO apaga nem reescreve nada da
// conduta. Apagar o último item destruiria orientação legítima ("Reavaliar em
// 48 h na UBS" é correto em alta) e reescrever seria o servidor inventando
// texto clínico. Ela só acende um aviso para a IA.
//
// CUIDADO COM FALSO POSITIVO: exige o verbo JUNTO da palavra alta ("dar alta",
// "vou dar alta", "alta para casa", "libero o paciente"). Assim, "pressão alta",
// "febre alta", "dose alta" e "alta suspeita" NÃO disparam. Também aceita
// "mando para casa" / "vai para casa".
function medicoDeclarouAlta(texto) {
    const t = semAcento(String(texto || ''));
    if (!t.trim()) return false;
    const padroes = [
        /\b(vou\s+)?(dar|dou|darei|dei)\s+(a\s+)?alta\b/,
        /\balta\s+(para|pra)\s+(casa|domicilio)\b/,
        /\balta\s+(hospitalar|medica|ambulatorial|com\s+receita|com\s+orientac)/,
        /\bde\s+alta\b/,
        /\balta\s+ap[oó]s\b/,
        /\b(vou\s+)?(liberar|libero|liberei|liberado|liberada)\s+(o\s+|a\s+)?(paciente|pcte|pct)\b/,
        /\b(vou\s+)?(mandar|mando)\s+(o\s+|a\s+)?(paciente\s+)?(para|pra)\s+casa\b/,
        /\b(paciente\s+)?(vai|ira)\s+(para|pra)\s+casa\b/
    ];
...
function preferirPrimeiraPessoaConduta(txt) {
    ...
    const trocas = [
        // v8.0: SAÍRAM daqui os verbos que a v8.0 passou a PROIBIR na conduta
        // (administrar, fazer, aplicar, puncionar, monitorizar, coletar,
        // transferir). Não faz sentido o servidor converter à força um verbo
        // para uma forma proibida — antes ele "consertava" o infinitivo
        // "Administrar dipirona" para "Administro dipirona", que descreve um
        // ato da enfermagem. Se a IA escorregar nesses verbos, agora o texto
        // fica visível ao médico em vez de ser normalizado pelo servidor.
    ...
    // v8.0 — O ÚLTIMO ITEM DA CONDUTA FICA NO INFINITIVO. Esta função estava
    // DESFAZENDO a regra nova: a IA escrevia "Reavaliar após as medicações" e
    // o servidor convertia para "Reavalio", nos três motores. O último item de
    // fecho (reavaliação/acompanhamento) agora é preservado.
    const linhas = txt.split('\n');
    let idxUltimoItem = -1;
    for (let i = linhas.length - 1; i >= 0; i--) {
        if (/^\s*(?:[-•*]\s*|\d+[.)]\s*)?\S/.test(linhas[i]) && linhas[i].trim() !== '') { idxUltimoItem = i; break; }
    }
    // v8.2: entraram "orientar" e "encaminhar". Com a ALTA já decidida pelo
    // médico, o fecho da conduta deixa de ser a reavaliação na UPA e passa a
    // ser a orientação de seguimento ("Orientar sinais de alarme e retorno...",
    // "Encaminhar para seguimento na unidade básica"). Sem esta linha, o
    // servidor convertia esse fecho para "Oriento"/"Encaminho" e desfazia a
    // regra do último item no infinitivo — o mesmo erro que a v8.0 corrigiu
    // para "Reavaliar". Vale SÓ no último item; no meio da conduta, "orientar"
    // continua virando "oriento" normalmente.
    const VERBOS_DE_FECHO = /^\s*(?:[-•*]\s*|\d+[.)]\s*)?(reavaliar|avaliar|acompanhar|observar|conferir|vigiar|aguardar|manter|orientar|encaminhar)\b/i;
    return linhas.map((linha, i) => {
        // No último item, se o verbo de abertura for um verbo de fecho no
        // infinitivo, ele FICA como está — é exatamente o formato pedido.
        if (i === idxUltimoItem && VERBOS_DE_FECHO.test(linha)) return linha;
...
// PEDIDO DE EXAME COM TRAÇOS, NUNCA NUMERADO (v8.0 — fatia J).
// Regra do médico: quem lê o pedido é o PACIENTE, e uma lista numerada faz a
// quantidade de exames parecer maior do que é ("são 12 exames?!"). A regra já
// está no prompt, mas escapava por motor — no teste de 07/set o Flash numerou
// e os outros dois não. Esta função é o suspensório: troca o marcador de
// numeração por traço APENAS nos campos de pedido de exame.
//
// POR QUE AQUI É SEGURO (e no verbo da conduta não era): trocar "1." por "-"
// no começo de um item de lista não muda sentido clínico nenhum. Trocar um
// verbo, sim, poderia inverter o sentido da frase.
//
// CUIDADOS (contra-exemplos cobertos pelo teste):
//  - só age no INÍCIO da linha (com ou sem espaços antes);
//  - exige o padrão "número + . ou ) + espaço" — "10 mL de SF 0,9%" NÃO vira
//    traço, porque não tem ponto/parêntese depois do número;
//  - "2.5 mg" também não casa (depois do ponto vem dígito, não espaço);
//  - NÃO é usada na prescrição interna, na prescrição de internação nem na
//    receita, onde a numeração é desejada.
function tracosNoPedidoExame(txt) {
    if (!txt || typeof txt !== 'string') return txt;
    return txt.replace(/^([ \t]*)\d{1,2}[.)]\s+/gm, '$1- ');
}
...
//  v8.1.1 — REPARO DE QUEBRA DE LINHA CRUA DENTRO DE STRING.
//  Causa provável do "JSON inválido" intermitente SÓ no Claude: o Gemini roda em
//  modo JSON nativo (responseMimeType), que torna esse erro impossível; o Claude
//  não tem esse modo e escreve o JSON à mão. E o nosso próprio prompt manda
//  "usar quebra de linha real" no exame físico e na conduta — o que, tomado ao
//  pé da letra DENTRO de uma string JSON, produz um arquivo inválido. Daí o
//  padrão de falhar numa tentativa e passar na seguinte, com o mesmo relato.
//  Esta função percorre o texto caractere a caractere, sabendo quando está
//  dentro de uma string (respeitando a barra invertida de escape), e converte a
//  quebra de linha crua no escape correto. É DETERMINÍSTICA e NÃO APAGA NADA:
//  nenhum caractere de conteúdo é descartado, só reescrito na forma válida.
function repararControlesEmStrings(txt) {
    let saida = '';
    ...
    for (let i = 0; i < txt.length; i++) {
        ...
        if (c === '"') { dentroDeString = !dentroDeString; saida += c; continue; }
        if (dentroDeString) {
            if (c === '\n') { saida += '\\n'; continue; }
            if (c === '\r') { saida += '\\r'; continue; }
            if (c === '\t') { saida += '\\t'; continue; }
            if (c < ' ') { continue; }   // outro caractere de controle: descartado
        }
        saida += c;
    }
    return saida;
...
//  v8.2 — REPARO DE ASPA SOLTA DENTRO DE STRING (segunda causa do "JSON
//  inválido" do Claude). O reparo da v8.1.1 cobre a QUEBRA DE LINHA crua; falta
//  o outro escorregão clássico de quem escreve JSON à mão: a aspa dupla no meio
//  do texto sem a barra invertida. É o caso das duas falhas registradas no log
//  de 11/set — respostas que começam em {"discussao": e terminam em
//  "relatorio": ""} , ou seja, inteiras, e ainda assim não abrem.
//  COMO DECIDE O QUE É FECHO E O QUE É CONTEÚDO: estando DENTRO de uma string,
//  uma aspa só pode ser o fim dela se o próximo caractere que não for espaço
//  for um dos que a gramática do JSON admite ali — vírgula, dois-pontos, chave
//  ou colchete de fecho — ou se o texto acabar. Qualquer outra aspa é conteúdo
//  e recebe a barra invertida.
//  NÃO APAGA NADA: só acrescenta a barra que faltava. Roda SÓ depois de todas
//  as leituras normais falharem, então não tem como estragar resposta que já
//  estava boa.
function repararAspasSoltas(txt) {
    let saida = '';
    let dentroDeString = false;
    let escapado = false;
    for (let i = 0; i < txt.length; i++) {
        ...
        if (!dentroDeString) { dentroDeString = true; saida += c; continue; }
        // Estamos dentro de uma string e apareceu uma aspa: fecha ou é conteúdo?
        let j = i + 1;
        while (j < txt.length && /\s/.test(txt[j])) j++;
        const seguinte = j < txt.length ? txt[j] : '';
...
//  v8.2 — guarda a MENSAGEM do último erro de leitura de JSON, só para o log do
//  Render. Não entra em resposta nenhuma e não guarda conteúdo de paciente.
let ultimoErroJson = '';
...
function extrairJson(txt) {
    ...
    if (ini !== -1 && fim !== -1 && fim > ini) {
        recorte = limpo.slice(ini, fim + 1);
        try { return JSON.parse(recorte); } catch (e) {}
    }
    // v8.1.1: últimas tentativas, agora com o reparo das quebras de linha cruas.
    if (recorte !== null) {
        try {
            const d = JSON.parse(repararControlesEmStrings(recorte));
            console.log('JSON recuperado pelo reparo de quebra de linha crua.');
            return d;
        } catch (e) {}
    }
    try {
        const d = JSON.parse(repararControlesEmStrings(limpo));
        console.log('JSON recuperado pelo reparo de quebra de linha crua.');
        return d;
    } catch (e) {}
    // v8.2: última linha de defesa — aspa solta dentro do texto. Aplicada
    // SEMPRE junto com o reparo de quebra de linha, porque as duas falhas
    // costumam vir na mesma resposta.
    const candidatos = recorte !== null ? [recorte, limpo] : [limpo];
    for (const alvo of candidatos) {
        try {
            const d = JSON.parse(repararAspasSoltas(repararControlesEmStrings(alvo)));
            console.log('JSON recuperado pelo reparo de aspa solta.');
            return d;
        } catch (e) {}
    }
    // Diagnóstico: guarda a mensagem do erro final para o log de quem chamou.
    try {
        JSON.parse(recorte !== null ? recorte : limpo);
    } catch (e) {
        ultimoErroJson = e && e.message ? e.message : String(e);
    }
    return null;
}
...
//  v8.2: 'modeloForcado' (opcional) permite chamar um nome de modelo específico
//  — é o que a escada de tentativas usa para experimentar o Flash de reserva.
//  Sem ele, tudo funciona exatamente como antes.
async function chamarGemini(promptFinal, modeloId, usarBusca, imagens, sinal, modeloForcado) {
    const modelToUse = modeloForcado || (modeloId === 'flash' ? MODELO_RAPIDO : MODELO_PROFUNDO);
    const configModelo = { model: modelToUse };
    if (usarBusca) {
        configModelo.tools = [{ googleSearch: {} }];
    }
    const model = genAI.getGenerativeModel(configModelo);
    // v7.8: SEM 'temperature'. Antes ia 0.4, para Flash e Pro. O Google
    // recomenda manter o padrão (1.0) nos modelos Gemini 3 — abaixo disso pode
    // dar looping ou piorar o desempenho. Retirado dos dois por decisão do
    // médico. Efeito esperado: o texto varia um pouco mais entre gerações do
    // mesmo relato. As travas do servidor (pós-processamento, diluições,
    // zeramento dos campos de internação) são determinísticas e não mudam.
    // v8.0.2 — TETO DE SAÍDA. Era 8192, herança antiga: o gemini-3.8-flash e o
    // gemini-2.5-pro aceitam 65.536. Com o modo internação ligado, o documento
    // inteiro (prontuário + 3 campos da AIH + relatório de internação +
    // prescrição de internação + relatório APS + pedido de exame) passou de
    // 10.000 tokens de saída — medido pelo custo da geração do Claude no teste
    // de campo de 07/set — e o Flash devolveu "JSON inválido" DUAS vezes, aos
    // 26,2 s e 23,6 s: a resposta vinha cortada no meio e o extrator recebia um
    // JSON sem fecho.
    // AGRAVANTE: nos modelos Gemini 3 o maxOutputTokens é orçamento COMBINADO
    // de pensamento + resposta (comportamento do servidor do Google, diferente
    // do que a documentação diz). Como o 3.8-flash pensa em nível médio por
    // padrão e isso não se desliga, parte do teto ia para o pensamento.
    // SUBIR O TETO NÃO CUSTA NADA sozinho: só é cobrado o que o modelo
    // realmente usar. O que ele faz é impedir o corte no meio do documento.
    ...
    // v7.9 — CUSTO: o Gemini 2.5-pro pensa por padrão e NÃO dá pra desligar
    // (o Flash 3.8 ainda não foi confirmado). O token de pensamento
    // ("thoughtsTokenCount") vem SEPARADO do "candidatesTokenCount" na
    // resposta — por isso soma os dois aqui: os dois são cobrados como saída.
...
//  v8.0: o toggle "pensamento extra" (v7.9) FOI REMOVIDO. Nos testes de campo
//  de 07/set, ligar a etapa extra de pensamento do Opus 5 não melhorou nada e
//  PIOROU a saída (receita domiciliar com antibiótico de casa em paciente que
//  ia internar, em 2 de 2 gerações; volta do condicional de alta na conduta; e
//  uma falha de JSON inválido). Agora a etapa extra vai SEMPRE desligada.
async function chamarClaude(promptFinal, imagens, sinal) {
    ...
    const resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
            model: MODELO_CLAUDE,
            // v8.0.2: teto subiu de 12000 para 24000. No teste de campo de
            // 07/set com internação ligada, esta mesma geração usou ~10.200
            // tokens de saída — 85% do teto anterior. Margem pequena demais
            // para um documento que pode crescer. O Opus 5 aceita até 128.000
            // e o teto só vira gasto se o modelo realmente usar os tokens.
            max_tokens: 24000,
            // v8.0: etapa extra de pensamento SEMPRE desligada (ver comentário
            // acima da função). Comportamento equivalente ao do antigo 4-8.
            thinking: { type: 'disabled' },
            // Reforço para o Claude devolver SÓ o JSON (sem texto em volta).
            // v8.1.1: acrescentada a regra da quebra de linha. O Gemini roda em
            // modo JSON nativo e não erra nisso; o Claude escreve o JSON à mão,
            // e o nosso prompt pede "quebra de linha real" no exame físico e na
            // conduta — o que, dentro de uma string, invalida o arquivo inteiro.
            system: 'Você responde EXCLUSIVAMENTE com um objeto JSON válido, '
                ...
                + 'REGRA CRÍTICA DE FORMATO: dentro de qualquer valor de texto, '
                + 'toda quebra de linha tem de ser escrita como os DOIS caracteres '
                + 'barra-invertida + n (\\n). NUNCA coloque uma quebra de linha de '
                + 'verdade dentro de uma string: isso invalida o JSON inteiro e a '
                + 'resposta é descartada.',
            // v8.1.2 — O PREFILL SAIU. A v8.1.1 mandava uma segunda mensagem,
            // { role: 'assistant', content: '{' }, para que a resposta já
            // começasse dentro do objeto. O claude-opus-5 RECUSA isso com HTTP
            // 400 ("does not support assistant message prefill; the conversation
            // must end with a user message") — ou seja, quebrava TODA chamada ao
            // Claude. A conversa agora termina em 'user', como a API exige.
            // Preâmbulo e cerca de código continuam cobertos pelo extrairJson.
            messages: [
    ...
    // Com pensamento ligado, blocos "thinking" vêm ANTES do texto — por isso
    // filtramos por 'type', nunca por posição. Isso já estava certo antes.
    // v8.1.2: sem prefill, a resposta já vem com o objeto inteiro. O "{" que a
    // v8.1.1 grudava aqui na frente TEM de sair junto — se ficasse, produziria
    // "{{" e todo JSON viria inválido.
    const texto = (data.content || [])
        .map(b => (b && b.type === 'text') ? b.text : '')
        .filter(Boolean)
        .join('\n');
    // v7.9 — CUSTO: no Claude, o "output_tokens" JÁ INCLUI os tokens de
    // pensamento (não são somados à parte, ao contrário do Gemini) — usar os
    // dois juntos aqui seria contar em dobro.
    const uso = data.usage || {};
    return {
        texto,
        // v8.1.1: 'end_turn' = terminou sozinho; 'max_tokens' = foi CORTADO no
        // teto. Sem isso, um JSON truncado era indistinguível de um JSON mal
        // escrito — e o log só dizia "JSON inválido", que não diagnostica nada.
        paradaPor: data.stop_reason || '',
...
//  e aplica o FALLBACK AUTOMÁTICO. Devolve { dados, motorUsado, caiuParaClaude, custo }.
...
async function gerarComFallback(promptFinal, modeloId, usarBusca, imagens, sinal) {
    if (modeloId === 'claude') {
        const r = await chamarClaude(promptFinal, imagens, sinal);
        return {
            dados: extrairJson(r.texto), motorUsado: 'claude', caiuParaClaude: false,
            textoBruto: r.texto, paradaPor: r.paradaPor,
            custo: calcularCustoBRL('claude', r.tokensEntrada, r.tokensSaida)
        };
    }
    // ------------------------------------------------------------------
    //  v8.2 — ESCADA DE TENTATIVAS ANTES DO CLAUDE.
    //  O 503 "high demand" do Google é pico de capacidade e, pela própria
    //  mensagem do Google, costuma passar em segundos. Até a v8.1.2, um único
    //  503 já mandava o atendimento para o Claude — trocando ~R$ 0,10 por
    //  ~R$ 0,90. Agora tentamos, nesta ordem:
    //    1ª) o modelo escolhido;
    //    2ª) o MESMO modelo, após 1,2 s;
    //    3ª) o MESMO modelo, após mais 3 s;
    //    4ª) só no Flash: o FLASH DE RESERVA (mesmo preço);
    //    5ª) Claude (como sempre foi).
    //  Tentativa que falha NÃO é cobrada. O que gasta é tempo — por isso o
    //  corte de 60 s: passou disso, não insiste mais, para não estourar o teto
    //  de 2 minutos da geração.
    // ------------------------------------------------------------------
    const comecouEm = Date.now();
    const AINDA_DA_TEMPO = () => (Date.now() - comecouEm) < 60000;
    const PAUSAS_MS = [1200, 3000];
    const empacotarGemini = (r) => ({
        dados: extrairJson(r.texto), motorUsado: 'gemini', caiuParaClaude: false,
        textoBruto: r.texto, paradaPor: r.paradaPor || '',
        custo: calcularCustoBRL(modeloId === 'flash' ? 'gemini_flash' : 'gemini_pro', r.tokensEntrada, r.tokensSaida)
    });
    //  Pausa que o botão "Cancelar geração" interrompe na hora.
    const esperar = (ms) => new Promise((resolve, reject) => {
        if (sinal && sinal.aborted) return reject(new Error('AbortError'));
        const t = setTimeout(() => { limpar(); resolve(); }, ms);
        const aoAbortar = () => { clearTimeout(t); limpar(); reject(new Error('AbortError')); };
        const limpar = () => { if (sinal) try { sinal.removeEventListener('abort', aoAbortar); } catch (e) {} };
        if (sinal) try { sinal.addEventListener('abort', aoAbortar, { once: true }); } catch (e) {}
    });

    let ultimoErro = null;
    for (let tentativa = 0; tentativa <= PAUSAS_MS.length; tentativa++) {
        if (tentativa > 0) {
            if (!AINDA_DA_TEMPO()) break;
            console.log('Gemini sobrecarregado; nova tentativa ' + (tentativa + 1)
                + ' em ' + (PAUSAS_MS[tentativa - 1] / 1000) + 's (motor=' + (modeloId || 'pro') + ').');
            await esperar(PAUSAS_MS[tentativa - 1]);
        }
        try {
            return empacotarGemini(await chamarGemini(promptFinal, modeloId, usarBusca, imagens, sinal));
        } catch (error) {
            // v7.1: CANCELAMENTO vem antes de tudo. Se o médico cancelou, o erro
            // do Gemini é um aborto — e aborto NÃO é sobrecarga nem motivo para
            // insistir. Sem esta guarda, o cancelamento dispararia chamadas
            // novas (e pagas).
            if (ehAborto(error, sinal)) throw error;
            ultimoErro = error;
            // Erro que NÃO é sobrecarga (nome de modelo errado, chave inválida,
            // pedido malformado) não melhora com nova tentativa: sai do laço.
            if (!ehFalhaTemporariaGemini(error)) break;
        }
    }

    //  Degrau extra, só para o Flash: o modelo de reserva, mesmo preço.
    if (ultimoErro && ehFalhaTemporariaGemini(ultimoErro)
        && modeloId === 'flash' && MODELO_RAPIDO_RESERVA && AINDA_DA_TEMPO()) {
        try {
            console.log('Flash principal (' + MODELO_RAPIDO + ') segue sobrecarregado; '
                + 'tentando o Flash de reserva (' + MODELO_RAPIDO_RESERVA + '), mesmo preço.');
            const r = await chamarGemini(promptFinal, modeloId, usarBusca, imagens, sinal, MODELO_RAPIDO_RESERVA);
            const pacote = empacotarGemini(r);
            pacote.modeloReserva = MODELO_RAPIDO_RESERVA;
            console.log('Respondido pelo Flash de reserva (' + MODELO_RAPIDO_RESERVA + ').');
            return pacote;
        } catch (error) {
            if (ehAborto(error, sinal)) throw error;
            console.error('Flash de reserva também não respondeu. Detalhe:',
    ...
    {
        const error = ultimoErro || new Error('Falha desconhecida no Gemini.');
        if (ehAborto(error, sinal)) throw error;
        ...
        if (ehFalhaTemporariaGemini(error) && process.env.ANTHROPIC_API_KEY) {
            console.error('Gemini falhou (sobrecarga) em todas as tentativas; indo para o Claude. Detalhe:', error.message);
            const r = await chamarClaude(promptFinal, imagens, sinal);
            return {
                dados: extrairJson(r.texto), motorUsado: 'claude', caiuParaClaude: true,
                textoBruto: r.texto, paradaPor: r.paradaPor,
                custo: calcularCustoBRL('claude', r.tokensEntrada, r.tokensSaida)
            };
        }
...
app.post('/api/atendimento', limitarAbuso, async (req, res) => {
    ...
    try {
        ...
        // v8.1: na ABA INTERNAÇÃO o relato é OPCIONAL — o caso já está no BE.
        // Nas outras abas continua obrigatório.
        if (!beId || (!mensagem && tipoDocumento !== 'internacao')) {
        ...
        // v8.1 — ABA INTERNAÇÃO: terceiro tipo de documento, não mais um toggle.
        const ehInternacao = tipoDocumento === 'internacao';
        ...
        // v8.0 (fatia O.3): a busca é uma ferramenta do GEMINI. O Claude não tem
        // ferramenta de busca nenhuma — mandar o bloco "BUSCA ATIVADA" para ele
        // era pedir link a quem não pode procurar, e a IA acabava citando de
        // memória. Com o Claude selecionado, a busca é IGNORADA e a tela AVISA
        // (regra 15: nada de falha silenciosa).
        const buscaPedida = op.buscarFontes === true;
        const buscaBloqueadaClaude = buscaPedida && modeloId === 'claude';
        const usarBusca = buscaPedida && modeloId !== 'claude';
        ...
        // v8.1: quem manda agora é a ABA, não o toggle. O toggle 🏥 saiu da
        // tela; se uma tela antiga ainda mandar op.internacao, ele é ignorado.
        const internacao = ehInternacao;
        ...
        let tipoAtendimento = ehInternacao
            ? 'INTERNAÇÃO (prontuário único de internação + documentos da AIH)'
            : ehEvolucao
                ? 'EVOLUÇÃO (reavaliação — texto curto + conduta)'
                : 'PRONTUÁRIO (documento completo)';

        // v8.1 — regra 15 (nada de falha silenciosa): na aba Internação, se não
        // há histórico, não há anexo e não há instrução escrita, não existe de
        // onde tirar o caso. Melhor recusar com clareza do que deixar a IA
        // inventar um paciente.
        if (ehInternacao
            && (!historicoPaciente || historicoPaciente.length === 0)
            && fotos.length === 0
            && !String(mensagem || '').trim()) {
            return res.status(400).json({ erro: 'Aba Internação: este BE não tem nada guardado nas últimas 72 h, não há anexo e o campo de instrução está vazio. Não há de onde montar o documento. Anexe o exame ou a evolução, ou escreva o caso no campo de instrução.' });
        }

        // Na aba Internação o campo de instrução pode vir vazio de propósito.
        const mensagemPrompt = (ehInternacao && !String(mensagem || '').trim())
            ? '(sem instrução adicional — monte o documento a partir do histórico do BE e dos anexos)'
            : mensagem;
        ...
        if (ehInternacao) {
            instrucoesAcao += '- PRONTUÁRIO ÚNICO DE INTERNAÇÃO: preencha TODOS os campos de "prontuario", mais "exames_complementares" e "cid", consolidando o caso INTEIRO (admissão + evoluções + exames). Deixe "evolucao" VAZIA.\n';
            instrucoesAcao += '- DOCUMENTOS DE INTERNAÇÃO (campos 20/21/22 do laudo de AIH e prescrição de internação), conforme o bloco MODO INTERNAÇÃO mais abaixo.\n';
        } else if (ehEvolucao) {
        ...
        // v8.2 — ALTA JÁ DECIDIDA PELO MÉDICO. Detecção determinística no
        // relato: quando o médico escreve que vai dar alta / liberar o
        // paciente, a conduta TEM que registrar isso e NÃO pode fechar com
        // reavaliação nesta UPA. Na aba Internação não faz sentido e não entra.
        const altaDeclarada = !ehInternacao && medicoDeclarouAlta(mensagem);
        if (altaDeclarada) {
            instrucoesAcao += '- ⚠️ O MÉDICO JÁ DECIDIU A ALTA NESTE ATENDIMENTO (está escrito no relato). '
                + 'Aplique o CASO B da regra da conduta: registre a alta em item próprio e assertivo '
                + '("Dou alta com receita e orientações..."), e feche a conduta com a orientação de '
                + 'seguimento FORA da UPA. É PROIBIDO que o último item seja "Reavaliar", "Reavalio", '
                + '"Mantenho em observação", "Observar" ou "Aguardar" — o paciente está indo embora, '
                + 'não há reavaliação nesta unidade para acontecer depois.\n';
            console.log('Relato declara ALTA: instrução do CASO B da conduta incluída no prompt.');
        }
        if (op.relatorio) {
        ...
        // v8.0 (fatia O.1): com o toggle DESLIGADO nada sobre este pedido entra
        // no prompt — nem a chave no esquema, nem o item do dicionário, nem uma
        // linha "NÃO gere...". Instrução negativa ocupa atenção e não produz
        // nada. A trava do servidor (que zera o campo) continua valendo.
        if (exameUnidade) {
...
`;
        ...
        // v8.0 (fatias O.1 e O.2): mesmo mecanismo da internação, agora para os
        // pedidos de exame e para o campo "fontes". Chave que existe no esquema
        // acaba preenchida — foi assim que o "fontes" passou a imprimir
        // "FONTES CONSULTADAS" com diretrizes citadas de memória, com a busca
        // DESLIGADA (atribuição fabricada em documento clínico).
        // Cada bloco condicional COMEÇA com vírgula e NÃO termina com vírgula,
        // para o esquema continuar sendo JSON válido em qualquer combinação de
        // toggles (inclusive todos desligados).
        const chavesExames = (exameUnidade ? ',\n  "exame_unidade": ""' : '')
                           + (exameExterno ? ',\n  "exame_externo": ""' : '');
        const chaveFontes = usarBusca ? ',\n  "fontes": ""' : '';

        // v8.0 (fatia O.1): os itens do dicionário de campos dos pedidos de
        // exame só entram no prompt quando o toggle correspondente está ligado.
        const itemExameUnidade = !exameUnidade ? '' : `7. "exame_unidade": texto do pedido de exame(s) a serem realizados NA PRÓPRIA
   UPA. Escreva como um pedido médico real (cabeçalho curto + lista dos exames).
   FORMATO DA LISTA: os exames vão em lista com TRAÇOS ("- Hemograma completo"),
   UM POR LINHA. É PROIBIDO numerar ("1.", "2.", "1)", "a)"). Quem lê este papel
   é o paciente, e a lista numerada faz a quantidade parecer maior do que é.

`;
        const itemExameExterno = !exameExterno ? '' : `8. "exame_externo": texto de solicitação de exame(s) para o paciente realizar
   ...
   a indicação clínica quando pertinente. FORMATO DA LISTA: TRAÇOS, um exame por
   linha, NUNCA numerada — a mesma regra do pedido na unidade.
   ATENDER AO QUE O MÉDICO PEDIU: quando ele indicar no relato o que o pedido
   deve conter — o CID, a justificativa clínica, o exame específico, o serviço
   de destino —, INCLUA exatamente o que foi pedido.
...
`;
  ...
  "prescricao_internacao": ""`;
            blocoInternacao = `
════════════════ MODO INTERNAÇÃO — ABA PRÓPRIA (v8.1) ════════════════
O médico JÁ DECIDIU internar. Esta geração NÃO é uma consulta nova: é a
CONSOLIDAÇÃO do caso que já existe neste BE.

DE ONDE VEM O CONTEÚDO, nesta ordem:
  1. HISTÓRICO DO BE (prontuário de admissão, evoluções e exames registrados).
  2. ANEXOS (fotos/PDFs de exames, prontuários e evoluções).
  3. MENSAGEM DO MÉDICO agora — instrução curta que PREVALECE sobre as duas
     anteriores quando houver conflito: é ela que traz a decisão de hoje.
NÃO invente etapa, dado ou resultado que não esteja em nenhuma das três fontes.
Se faltar informação para um campo, deixe-o vazio e diga isso na "discussao".

O DOCUMENTO PRINCIPAL É O PRONTUÁRIO ÚNICO DE INTERNAÇÃO: os campos de
"prontuario" (história clínica, história pregressa, exame físico, sinais
vitais, hipótese diagnóstica e conduta), mais "exames_complementares" e "cid",
preenchidos com o caso INTEIRO — do que trouxe o paciente até o que se decidiu
agora. Não é o recorte de uma consulta: é o documento que segue com ele.
NÃO existe campo separado de relatório de internação; estes campos SÃO ele.
O campo "evolucao" fica SEMPRE VAZIO nesta geração.
Na "conduta" desse prontuário, a linha da internação é EXATAMENTE
"Solicito internação hospitalar e preencho a documentação" — nunca "interno o
paciente", "aciono a vaga" ou "transfiro".
...
A) "aih_sinais_sintomas" (campo 20 do laudo de AIH): quadro clínico OBJETIVO —
   ...
   TAMANHO: NO MÁXIMO 6 LINHAS, cerca de 90 palavras. É um formulário oficial de
   espaço curto — quem lê quer o quadro, não o prontuário de novo. NÃO repita
   antecedentes, NÃO liste exames, NÃO justifique a internação (isso é o campo
   21) e NÃO descreva o exame físico sistema por sistema.
...
B) "aih_condicoes" (campo 21): POR QUE este paciente NÃO pode ser tratado
   ...
   TAMANHO: NO MÁXIMO 5 LINHAS, cerca de 80 palavras. Só o argumento da
   internação. NÃO repita o quadro clínico do campo 20 nem transcreva exames.

C) "aih_exames" (campo 22): SOMENTE exames REALMENTE realizados e relatados,
   com valores e unidades exatos. Só a TRANSCRIÇÃO — sem comentário, sem
   interpretação e sem conduta.
...
D) "prescricao_internacao": prescrição da internação, nesta ordem de blocos
   ...
   PROIBIDO ESCREVER DILUIÇÃO (v8.0): vale aqui a mesma regra da
   "prescricao_interna" — nada de diluente, volume, reconstituição ou tempo de
   infusão. O sistema anexa a diluição a partir da tabela conferida e não
   escreve por cima do que você escrever.
...
- MEDICAÇÃO FORA DA PADRONIZAÇÃO (SUS): se o médico prescreveu um remédio que
  ...
  ele não está padronizado. Você SÓ pode contrariar/substituir uma prescrição do
  médico por motivo de SEGURANÇA (interação medicamentosa, alergia relatada,
  dose perigosa, contraindicação) — NUNCA pela simples ausência no SUS.
  COMO SINALIZAR (v8.1.1 — mudou): este médico JÁ SABE o que é e o que não é
  padronizado na unidade dele; avisar disso não agrega nada e estava ocupando a
  discussão inteira.
   • RECEITA DOMICILIAR: NÃO sinalize NADA. Remédio de farmácia não sai do
     estoque da UPA — é esperado que não esteja na lista. Silêncio total.
   • PRESCRIÇÃO EXECUTADA DENTRO DA UNIDADE (prescricao_interna e
     prescricao_internacao): pode sinalizar, mas em UMA ÚNICA LINHA CURTA, no
     FIM da "discussao", sem caixa alta e sem justificar. Ex.: "Fora da
     padronização daqui: amoxicilina+clavulanato." Nada além disso.
  Isto NUNCA é assunto do bloco principal da discussão.
...
REGRAS DE SAÍDA (responda em JSON puro com EXATAMENTE estas chaves):
{
  ...
  "prescricao_interna": "",
  "receita": "",
  "relatorio": ""${chavesExames}${chavesInternacao}${chaveFontes}
}
...
1. "discussao": este é o campo de RACIOCÍNIO CLÍNICO — o mais importante para o
   ...
   ESTRUTURA (v8.1.1) — o assunto principal é A CONDUTA DESTE MÉDICO:

   NÍVEL 1 — abra com a linha "SOBRE A SUA CONDUTA:" (este rótulo exato). Este
   bloco existe SEMPRE. Ele responde, nesta ordem, a três perguntas, citando
   pelo NOME os itens que o médico prescreveu, pediu ou decidiu:
     (a) VOCÊ CONCORDA com a conduta descrita? Diga com todas as letras. Se
         concorda, uma linha basta ("Concordo com a conduta: dose, via e
         intervalo adequados ao caso"); não fabrique ressalva para parecer útil.
         Se discorda de algum ponto, diga qual e POR QUÊ.
     (b) VOCÊ ACRESCENTARIA alguma coisa — medicação, exame, medida de suporte,
         orientação? Diga o quê e por quê. Se não acrescentaria nada, diga isso.
     (c) VOCÊ RETIRARIA OU AJUSTARIA alguma coisa? Medicação desnecessária,
         dose, via, intervalo, duração, interação, ajuste para função renal,
         idade ou alergia. Se não retiraria nada, diga isso.
   Escreva em prosa de colega, não em formulário: pode juntar as três respostas
   em poucas linhas se o caso for simples. Sem teto rígido de linhas — o limite
   é não escrever enchimento. Densidade não é comprimento.

   NÍVEL 2 — bloco OPCIONAL, encabeçado por "O QUE MAIS MUDA A CONDUTA:" (este
   rótulo exato), para o que não coube no nível 1 e ainda decide alguma coisa:
   alvo terapêutico particular deste paciente; achado do histórico ou do anexo
   que muda a decisão; o que mudaria se um exame vier alterado; risco que exige
   vigilância específica. Se não houver nada disso, NÃO escreva o bloco.

   NÍVEL 3 — bloco OPCIONAL, encabeçado por "DIFERENCIAIS:" (este rótulo
   exato), e SÓ quando o DIAGNÓSTICO estiver de fato EM ABERTO: achado que não
   fecha, dor torácica ou abdominal sem causa definida, sintoma neurológico,
   quadro que pode ser duas coisas diferentes. Para CADA diferencial diga o que
   no caso o FAVORECE ou AFASTA — raciocine, não liste — e inclua os "CAN'T
   MISS" daquele quadro e como descartá-los. ATENÇÃO: gravidade NÃO é
   ambiguidade. Pneumonia evidente em paciente hipoxêmico é GRAVE, mas o
   diagnóstico não está em aberto: nesse caso NÃO escreva este bloco. Em quadro
   clássico e coerente (pneumonia típica, IVAS viral, lombalgia mecânica sem red
   flag), também não: descartar TEP e síndrome coronariana num caso que não os
   sugere gasta espaço e não ajuda ninguém.

   O QUE NÃO É ASSUNTO DA DISCUSSÃO: se o medicamento consta ou não na
   padronização/no SUS. Isso é secundário, o médico já sabe, e tem lugar próprio
   (uma linha curta no fim — ver a regra da padronização acima). NUNCA abra a
   discussão com isso e nunca dedique um parágrafo a isso.
...
2c. "exames_complementares": o RESULTADO de exames complementares — ou seja,
   ...
   EVOLUÇÃO. Atenção: NÃO confundir RESULTADO com PEDIDO de exame — este campo
   é só para o RESULTADO já disponível.
...
Neurológico: ECG 15, pupilas isocóricas e fotorreagentes, pares cranianos preservados, força e sensibilidade preservadas e simétricas nos 4 membros, sem sinais de irritação meníngea"
   ...
   - "conduta": condutas e orientações. SEJA CONCISO — nada de "encher
     ...
     TEMPO VERBAL E VOZ — VERBOS DO MÉDICO, NÃO DA ENFERMAGEM (v8.0): use a
     PRIMEIRA PESSOA DO SINGULAR no presente, mas APENAS com verbos que
     descrevem atos do MÉDICO: "prescrevo", "solicito", "oriento", "encaminho",
     "mantenho", "indico", "reavalio". É PROIBIDO usar verbos que descrevem a
     EXECUÇÃO, que é da enfermagem: "instalo", "administro", "punciono",
     "ofereço", "coleto", "infundo", "faço" (no sentido de aplicar). O médico
     PRESCREVE oxigenoterapia; quem instala o cateter é a enfermagem. Escreva
     "Prescrevo oxigenoterapia 2-4 L/min se saturação < 94%", NUNCA "Instalo
     oxigenoterapia...". Também é proibido o particípio ("feito", "prescrito").
     INTERNAÇÃO — VERBO CORRETO: o médico da UPA SOLICITA a internação e
     PREENCHE a documentação; a vaga depende da coordenação de vagas e da
     conferência dos documentos, e NÃO está sob o controle de quem escreve.
     Escreva "Solicito internação hospitalar e preencho a documentação". É
     PROIBIDO escrever "interno o paciente", "aciono vaga", "transfiro para o
     hospital" ou qualquer forma que registre a internação como fato consumado.
     ÚLTIMO ITEM NO INFINITIVO: o último item da conduta é o item de FECHO e SÓ
     ele vai no INFINITIVO; os demais ficam na primeira pessoa. QUAL é o fecho
     depende de o desfecho já estar decidido ou não — veja os dois casos logo
     abaixo. Verbos aceitos no fecho: "Reavaliar...", "Acompanhar...",
     "Observar...", "Conferir...", "Orientar...", "Encaminhar...", "Manter...".
     CASO A — DESFECHO EM ABERTO (o médico NÃO disse o que vai fazer com o
     paciente): o fecho é a reavaliação na própria unidade — "Reavaliar após as
     medicações e os resultados dos exames."
     CASO B — O MÉDICO JÁ DECIDIU A ALTA: se o relato do médico diz que ele vai
     dar alta, liberar o paciente, mandar para casa ou que a alta já foi dada,
     isso é FATO DO ATENDIMENTO e TEM QUE CONSTAR NA CONDUTA. Neste caso:
       - É PROIBIDO fechar com "Reavaliar", "Reavalio", "Mantenho em
         observação", "Observar", "Aguardar" ou qualquer item que deixe o
         paciente na unidade. O paciente está indo embora: não há reavaliação
         nesta UPA para acontecer depois.
       - Registre a alta em item próprio, na primeira pessoa, junto do que foi
         entregue e do encaminhamento: "Dou alta com receita e orientações, e
         encaminho para seguimento na atenção primária."
       - O fecho (último item, infinitivo) passa a ser a orientação de
         seguimento fora da UPA: "Orientar sinais de alarme e retorno imediato
         ao serviço de urgência se piora." ou "Encaminhar para seguimento na
         unidade básica de saúde." Se o médico indicou retorno marcado em outro
         serviço, o fecho pode ser "Reavaliar em 48 horas na unidade básica de
         saúde" — reavaliação FORA da UPA é legítima; o que é proibido é
         devolver o paciente à observação de quem acabou de liberá-lo.
     PROIBIDO CONDICIONAL DE DESFECHO: a conduta registra o que está sendo
     feito AGORA. É PROIBIDO escrever critério de alta, de liberação ou de
     desfecho ANTECIPADO — nada de "se melhorar, alta", "alta se estabilizar",
     "entrego a receita se houver liberação", "libero após reavaliação". Isto
     vale para o CASO A (desfecho ainda não decidido), em que o desfecho vira
     uma NOVA evolução depois. Não confunda: alta que o médico JÁ decidiu e
     escreveu no relato NÃO é condicional — é o CASO B e tem que ser
     registrada de forma assertiva. Nada disso proíbe sinais de alarme nem
     orientação de retorno na receita domiciliar, que continuam normais, no
     lugar deles.
     NÃO REPETIR A LISTA DE EXAMES: a lista detalhada já sai na caixa de pedido
     de exame. Na conduta, cite os exames por CATEGORIA, em UM item só:
     "Solicito exames laboratoriais, radiografia de tórax e ECG." NUNCA enumere
     exame por exame aqui — nem entre parênteses. ERRADO: "Solicito exames
     laboratoriais (hemograma, PCR, ureia, creatinina, sódio, potássio) e
     radiografia de tórax (PA e perfil)". CERTO: "Solicito exames laboratoriais
     e radiografia de tórax." O detalhamento pertence ao pedido de exame.
     UM ITEM = UMA IDEIA: não empilhe quatro ações num item só para caber em 4
     linhas. ERRADO: "Solicito internação, preencho documentação, oriento
     suspensão da metformina, mantenho em observação e reavalio após os
     resultados." Se não couber, agrupe o que é da mesma natureza e corte o
     resto — o detalhe fino vai para a discussão.
     ...
     MODELO (CASO A — desfecho em aberto):
     "- Prescrevo na UPA: Tenoxicam 20 mg IM + Dipirona 1 g (2 mL) EV, agora.
- Solicito exames laboratoriais e radiografia de tórax.
- Prescrevo para casa: Naproxeno 500 mg de 12/12 horas por 5 dias e Metoclopramida 10 mg de 8/8 horas se náusea, e oriento sinais de alerta.
- Reavaliar após as medicações e os resultados dos exames."
     MODELO (CASO B — o médico já decidiu a alta):
     "- Prescrevo na UPA: Dipirona 1 g (2 mL) EV + Escopolamina 20 mg EV, agora.
- Prescrevo para casa: Dipirona 1 g de 6/6 horas se dor por 3 dias.
- Dou alta com receita e orientações, e encaminho para seguimento na atenção primária.
- Orientar sinais de alarme e retorno imediato ao serviço de urgência se piora."
     (campo 06)
...
4. "prescricao_interna": medicações usadas NA UPA. APENAS itens da lista desta
   ...
   a via (EV/IM) para o sistema localizar a diluição.
   PROIBIDO ESCREVER DILUIÇÃO (v8.0): você NUNCA escreve diluente, volume de
   diluição, reconstituição ou tempo de infusão nesta prescrição. Nada de
   "diluído em 100 mL de SF 0,9%", "em 250 mL de SG 5%", "reconstituir com
   10 mL de água", "correr em 30 minutos". Quem acrescenta a diluição é o
   próprio sistema, a partir de uma tabela conferida — e ele NÃO escreve por
   cima do que você escrever. Ou seja: uma diluição escrita por você APAGA a
   informação correta e o preparo sai pela metade (sem a reconstituição, por
   exemplo). Escreva apenas nome, dose, quantidade de ampolas/frascos, via e
   frequência, e PARE. ÚNICA EXCEÇÃO: se o próprio médico ditou a diluição no
   relato, reproduza exatamente o que ele disse.
   MODELO:
...
- Evitar esforço físico intenso até reavaliação."
   USO CONTÍNUO — NÃO PRESCREVER (v8.0, regra forte): é PROIBIDO incluir na
   receita domiciliar as medicações de USO CONTÍNUO do paciente
   (anti-hipertensivo, hipoglicemiante, estatina, psicotrópico, etc.), MESMO que
   elas apareçam no relato. Motivo: a lista que o paciente informa de boca
   frequentemente é imprecisa, e a renovação dessas medicações é competência de
   quem acompanha a trajetória clínica dele, na atenção primária — não do
   plantonista da urgência. ÚNICA EXCEÇÃO: o médico pedir EXPRESSAMENTE no
   relato ("renovar a receita de losartana", "colocar a metformina na receita").
   Aí sim, obedeça literalmente. Quando o uso contínuo tiver relevância clínica
   agora (suspender metformina, ajustar dose por função renal), isso vai na
   "discussao" e, se couber ao paciente, nas ORIENTAÇÕES — NUNCA vira receita.
   COERÊNCIA COM A INTERNAÇÃO (v8.0): quando a conduta indicar internação,
   transferência ou permanência na unidade, a receita NÃO prescreve tratamento
   de casa para a doença aguda — nada de antibiótico, corticoide ou esquema
   completo "para depois", porque quem vai tratar é o hospital. Se não sobrar
   nada legítimo para prescrever, a receita pode conter APENAS o bloco
   ORIENTAÇÕES, cujo PRIMEIRO item diz que a internação está indicada. Ex.:
   "- Necessária internação hospitalar neste momento; aguardar na unidade a
   definição da vaga."
...
6. "relatorio": parágrafo único, de médico para médico, começando com
   ...
   TAMANHO (v8.0.2): CURTO — no máximo 8 linhas, cerca de 130 palavras. Quem lê
   é um colega ocupado da atenção primária, que precisa saber o essencial em
   poucos segundos, não reler o prontuário inteiro.
   O QUE ENTRA: o motivo da vinda e o tempo de evolução; as comorbidades que
   importam para o seguimento; o diagnóstico; o que foi feito aqui; o desfecho
   (alta, internação solicitada, transferência); e — o mais importante — O QUE
   A APS PRECISA FAZER daqui em diante.
   O QUE NÃO ENTRA: o exame físico completo, a lista inteira de sinais vitais,
   a relação de todos os exames pedidos, nem a repetição literal dos campos do
   prontuário. Isso já está no prontuário e no relatório de internação; repetir
   aqui só faz o colega perder tempo procurando o que interessa.

${itemExameUnidade}${itemExameExterno}
...
${mensagemPrompt}`;
        ...
        function posProcessar(dados) {
            ...
            // --- Pedido de exame: numeração vira traço (v8.0, fatia J) ---
            //     Só nestes dois campos. A numeração da prescrição fica intacta.
            dados.exame_unidade = tracosNoPedidoExame(dados.exame_unidade);
            dados.exame_externo = tracosNoPedidoExame(dados.exame_externo);
            ...
            // v8.1: 'relatorio_internacao' fica na lista SÓ para ser zerado — o
            // campo saiu do esquema (os campos do prontuário são o relatório).
            ...
            if (!internacao) {
                ...
                dados.relatorio_internacao = '';
                // v8.1: na aba Internação o documento é de PRONTUÁRIO. Se a IA
                // escrever evolução por hábito, o servidor zera — senão a tela
                // mostraria dois documentos concorrentes para o mesmo caso.
                dados.evolucao = '';
            }
            ...
            // v8.1.1: a RECEITA DOMICILIAR saiu da conferência. Remédio de
            // farmácia não sai do estoque da UPA — apontar que não está na
            // padronização era ruído garantido em toda receita, e o médico já
            // sabe. Continuam conferidas as duas prescrições EXECUTADAS DENTRO
            // da unidade, onde a informação ainda serve para alguma coisa.
            const textoConferir = [dados.prescricao_interna,
                dados.prescricao_internacao].filter(Boolean).join('\n');
            const alertas = conferirMedicamentos(textoConferir, listaUnidade);
            if (alertas.length > 0) {
                const aviso = '\n\nFora da padronização desta unidade (conferência grosseira, confira você mesmo): '
                    + alertas.join(', ') + '.';
            ...
            // v8.0 (fatia O.2): só quando a busca REALMENTE rodou. Antes, a
            // chave "fontes" existia sempre no esquema, a IA a preenchia de
            // memória e o sistema imprimia "FONTES CONSULTADAS" sem nada ter
            // sido consultado — atribuição fabricada em documento clínico.
            if (usarBusca && dados.fontes && dados.fontes.trim()) {
                dados.discussao = (dados.discussao || '')
                    + '\n\n📚 FONTES CONSULTADAS:\n' + dados.fontes.trim();
            }
            return dados;
        ...
        if (compararIAs) {
            ...
            try {
                ...
                [rGemini, rClaude] = await Promise.all([
            ...
            const dadosGemini = posProcessar(extrairJson(rGemini.texto));
            const dadosClaude = posProcessar(extrairJson(rClaude.texto));
            if (!dadosGemini || !dadosClaude) return erroJsonInvalido();

            // v7.9 — custo de cada versão gerada (a do resumo, mais abaixo,
            // ainda entra somada ao total do Claude).
            const custoGemini = calcularCustoBRL('gemini_pro', rGemini.tokensEntrada, rGemini.tokensSaida);
            let custoClaude = calcularCustoBRL('claude', rClaude.tokensEntrada, rClaude.tokensSaida);
...
${JSON.stringify(dadosClaude)}`;
                const respCmp = await fetch('https://api.anthropic.com/v1/messages', {
                    ...
                    body: JSON.stringify({
                        model: MODELO_CLAUDE,
                        max_tokens: 2048,
                        // v7.9: o resumo é uma tarefa simples (comparar dois JSON já
                        // prontos) — mantém sempre sem pensamento extra, mesmo com o
                        // toggle ligado, pra não gastar à toa numa etapa secundária.
                        thinking: { type: 'disabled' },
                ...
                if (respCmp.ok) {
                    ...
                    const usoResumo = d.usage || {};
                    custoClaude = somarCustoBRL(custoClaude, calcularCustoBRL('claude', usoResumo.input_tokens || 0, usoResumo.output_tokens || 0));
                }
            } catch (e) {
            ...
            // Devolve as DUAS versões + o resumo + o custo dos dois motores.
            // A escolha (e o salvamento) é feita depois, pela tela, na rota
            // /api/escolher.
            return res.json({
                _comparacao: true,
                resumoDiferencas,
                ...
                _custo: { gemini: custoGemini, claude: custoClaude }
            });
        }
        ...
        if (!dados) {
            // v8.1.1 — o log antigo dizia só "JSON inválido", o que não permitia
            // distinguir resposta CORTADA no teto de resposta MAL ESCRITA. Agora
            // sai o motor, o motivo da parada, o tamanho e as pontas do texto.
            // Nada disso é conteúdo clínico do paciente além do que a própria IA
            // escreveu, e o começo/fim de um JSON são chaves e rótulos.
            // v8.2 — o log passou a dizer TAMBÉM a mensagem do erro de leitura e
            // a mostrar o trecho onde a leitura travou. Sem a posição, uma
            // resposta que começa e termina certa (como as duas de 11/set) não
            // dava para diagnosticar: o defeito estava no MEIO do texto.
            const bruto = String(saida.textoBruto || '');
            let trecho = '';
            const posicao = /position\s+(\d+)/i.exec(ultimoErroJson || '');
            if (posicao) {
                const p = parseInt(posicao[1], 10);
                trecho = ' trecho=' + JSON.stringify(bruto.slice(Math.max(0, p - 90), p + 90));
            }
            console.error('IA devolveu JSON inválido.'
                + ' motor=' + (saida.motorUsado || '?')
                + ' parada=' + (saida.paradaPor || 'nao_informado')
                + ' tamanho=' + bruto.length
                + ' erro=' + JSON.stringify(ultimoErroJson || 'nao_informado')
                + trecho
                + ' inicio=' + JSON.stringify(bruto.slice(0, 120))
                + ' fim=' + JSON.stringify(bruto.slice(-120)));
            return erroJsonInvalido();
        }

        // v8.0 (fatia O.3): se a busca foi pedida mas o motor era o Claude, ela
        // foi ignorada — a tela precisa DIZER isso (regra 15).
        if (buscaBloqueadaClaude) {
            dados._avisoBusca = 'A busca em fontes só funciona no Gemini. Esta geração foi feita SEM busca.';
        }
        ...
        // v7.9: custo estimado em reais desta geração (null se não deu pra calcular).
        dados._custo = saida.custo;
        ...
        historicoPaciente.push({
            ...
            tipo: ehInternacao ? 'internacao' : ehEvolucao ? 'evolucao' : 'prontuario',
            fotos: fotos.length,
            quando: quando
        });
...
const VERSAO_SERVIDOR = 'v8.2';
app.get('/', (req, res) => res.send(
    'Servidor do Prontuario Rapido ' + VERSAO_SERVIDOR + ' no ar. '
    + 'Linhas esperadas no server.js: 3538.'
));
